...
function buildSystemPrompt(sport, mode, budget, risk, oddsData) {
  ...
  const oddsContext = oddsData && oddsData.length > 0
    ? `\n\n⚠️ LIVE ODDS DATA — THESE ARE THE ONLY REAL FIXTURES YOU ARE ALLOWED TO REFERENCE. DO NOT MENTION ANY OTHER MATCHES.\n${oddsData.map(g => {
        const bm = g.bookmakers?.[0];
        const market = bm?.markets?.find(m => m.key === "h2h");
        if (!market) return null;
        const outcomes = market.outcomes;
        const dateStr = new Date(g.commence_time).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" });
        return `• ${g.home_team} vs ${g.away_team} | ${dateStr}\n  ${outcomes.map(o => `${o.name} @ ${o.price?.toFixed(2)} (${((1/o.price)*100).toFixed(1)}% implied)`).join(" | ")}`;
      }).filter(Boolean).join("\n")}`
    : `\n\n⚠️ NO LIVE ODDS LOADED. Do NOT invent or mention any specific match, team, score, or fixture. Tell the user to load odds first via the 📊 LIVE ODDS tab before asking for picks.`;
...
- ⛔ NEVER invent, fabricate, or reference any match, team, score, fixture, or result that is not in the LIVE ODDS DATA above.
- ⛔ NEVER reference old matches, historical results, or past seasons as if they are upcoming.
- ✅ ONLY analyze the exact fixtures listed in the LIVE ODDS DATA section above.
- ✅ Always cite the exact odds from the live data when making picks.
- ✅ If no live odds are loaded, tell the user to load them first — do NOT invent fixtures.
